...
- [x] Discovery pulse feed — /events is now the pulse (cover, host + seal on the cover edge, attendee avatar stack, countdown chip "Starts in 3h"/"Tomorrow 19:00", price/Online chips, honest spots-left ≤5, sold-out); followed hosts float up then soonest-first ("nearest" awaits real geo data); original list intact at /events/all, cross-linked both ways; Events added to both navs; 9 seeded Melbourne events ([seed data — safe to delete] in descriptions)
- [ ] Waitlists on full events (also properly fixes the free-join capacity race)
- [x] Decaying-refund curve — ONE curve for bookings AND event tickets (replaces the P4 flexible/moderate/strict tiers; picker removed, policy columns vestigial pending cleanup migration): 100% ≥7d, linear slide to 50% floor, locked <48h; boundary/monotonicity math spot-checked. cancelEventTicket (curve refund → seat freed → remainder releases to creator via existing cron math); deleteEvent refunds all held tickets 100% first, blocked after any payout released; RefundTimeline visual ("Cancel now: full refund · locks {date}") on share page, event page (+ ticket-holder cancel), booking cancel preview
- [ ] Copy: remaining booking-flow + email copy → events language (bulk done in P5)
- [x] **Decided 16 Jul 2026: locks are dead.** All visibility gating removed (profiles always fully visible; verification gating unchanged); membership_tier survives as badges/perks only. Cleanup pass still owed: access_requests/profile_unlocks routes+tables, unlock-fee revenue line, visibility/lock_level columns
