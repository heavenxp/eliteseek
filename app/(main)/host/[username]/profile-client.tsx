    ...
    <div className="min-h-screen bg-[rgba(8,8,16,1)]">
      ...
      <div className="mx-auto max-w-2xl px-4">
        ...
        <div className="-mt-12 flex items-end justify-between md:-mt-14">
          ...
          <div className="mb-2 flex items-center gap-2">
            ...
            <div className="relative" ref={optionsRef}>
              ...
              {optionsOpen && (
                <div className="absolute right-0 top-11 z-30 min-w-[160px] overflow-hidden rounded-xl border border-[rgba(212,175,55,0.15)] bg-[rgba(12,12,24,0.97)] shadow-xl backdrop-blur-sm">
                  {isOwner ? (
                    <>
                      <OptionsItem href="/companion/posts/new">
                        <Icon name="plus" className="h-4 w-4" /> New Post
                      </OptionsItem>
                      <OptionsItem href="/account/settings">
                        <Icon name="camera" className="h-4 w-4" /> Edit Profile
                      </OptionsItem>
                    </>
                  ) : (
                    <>
                      {companion.booking_rate_hourly && isFullyVisible && (
                        <OptionsItem
                      ...
                      {companion.subscription_price && !isSubscribed && isFullyVisible && (
                        <div className="px-1 py-0.5" onClick={() => setOptionsOpen(false)}>
                          <SubscribeButton
                      ...
                      {isFullyVisible && (
                        <OptionsItem href={`/gifts?companion=${companion.id}`}>
                          <Icon name="star" className="h-4 w-4" /> Send Gift
                        </OptionsItem>
                      )}
                    </>
                  )}
        ...
        <div className="mt-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {companion.age && (
                <span className="ml-1.5 text-xl font-light text-muted/50">{companion.age}</span>
              )}
            </h1>
            {isSelect && (
            ...
            {isVerified && !isSelect && (
              <span className="badge-verified rounded-full px-2.5 py-0.5 text-xs">Verified</span>
            )}
          </div>
          ...
          {companion.average_rating && companion.total_reviews > 0 && (
            <div className="flex flex-col items-start">
              ...
              <span className="text-[10px] text-muted/40">
                {companion.total_reviews} review{companion.total_reviews !== 1 ? "s" : ""}
              </span>
            </div>
          )}
        </div>
          ...
          >
            ...
            {companion.available_from && (
              <span className="flex items-center gap-1.5">
                ...
                {companion.available_from && companion.location
                  ? `${companion.location} · `
                  : ""}
        ...
        <div className="mt-2 flex items-center gap-1.5">
          ...
          />
          <span
            className={`text-xs ${companion.is_available ? "text-emerald-400" : "text-muted/40"}`}
            style={{ fontFamily: "var(--font-dm-sans)" }}
          >
            {companion.is_available ? "Available for bookings" : "Not taking bookings"}
          </span>
        </div>
        ...
        <div className="pb-20 pt-4">
          {activeTab === "posts" && (
            <PostsTab posts={feedPosts} />
          )}
          {activeTab === "media" && (
            <MediaTab
              items={mediaItems}
              isSubscribed={isSubscribed}
              hasUnlocked={!isFullyVisible ? false : true}
            />
          )}
          {activeTab === "videos" && (
            <VideosTab
    ...
    <div className="mt-4 rounded-2xl border border-[rgba(212,175,55,0.15)] bg-[rgba(212,175,55,0.04)] px-4 py-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="text-sm font-medium text-foreground/80" style={{ fontFamily: "var(--font-dm-sans)" }}>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {(lockStatus === "locked" || (lockStatus === "elite_only" && clientTier === "silver")) &&
            ...
            {accessRequestStatus === "declined" && (
              <span className="text-xs text-red-400/70" style={{ fontFamily: "var(--font-dm-sans)" }}>
...
function PostsTab({ posts }: { posts: FeedPostPreview[] }) {
  if (posts.length === 0) {
    return (
...
function FeedPostCard({ post }: { post: FeedPostPreview }) {
  return (
    <div className="glass-card overflow-hidden rounded-2xl px-4 py-4">
      <p className="text-sm leading-relaxed text-foreground/80" style={{ fontFamily: "var(--font-dm-sans)" }}>
        {post.content}
      </p>
      {post.image_url && (
        // eslint-disable-next-line @next/next/no-img-element
        ...
        return (
          <div key={i} className="relative aspect-square overflow-hidden rounded-lg">
            ...
            <img
              src={item.url}
              alt=""
              className={`h-full w-full object-cover transition-all ${locked ? "blur-xl scale-110 brightness-50" : ""}`}
            />
            {locked && (
        ...
        return (
          <div key={i} className="relative overflow-hidden rounded-2xl">
            <video
              ...
              muted
              playsInline
              className={`aspect-video w-full object-cover transition-all ${locked ? "blur-xl brightness-50" : ""}`}
            />
            {locked && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                ...
                {item.post.is_ppv && item.post.ppv_price && (
                  <span className="rounded-full border border-[rgba(212,175,55,0.3)] bg-[rgba(8,8,16,0.7)] px-3 py-1 text-xs text-gold" style={{ fontFamily: "var(--font-dm-sans)" }}>
            ...
            {item.post.title && !locked && (
              <p className="mt-1.5 px-1 text-xs text-muted/50" style={{ fontFamily: "var(--font-dm-sans)" }}>
                {item.post.title}
              </p>
            )}
          </div>
